...
The Piper URDF is tracked, but its generated DAE resources are intentionally
local-only. Prepare them from the pinned official repository revision before
the first build:

```bash
cd /home/zhenyu/robonix
...
`model/piper_with_gripper.urdf`. Do not stage the generated `model/meshes/`
directory. See `system/soma/README.md` for the general resource path
convention and `MODEL_SOURCE.md` for provenance.
...
source ~/.bashrc
robonix-env
cd /home/zhenyu/robonix/examples/piper_vitals
rbnx build
```
...
```text
ssh -N -T -o ExitOnForwardFailure=no -L 17860:127.0.0.1:7860 DDST_72
```
...
`agilexrobotics/agx_arm_urdf` repository. Binary DAE resources stay local. See
`MODEL_SOURCE.md`, `model/manifest.json`, and
