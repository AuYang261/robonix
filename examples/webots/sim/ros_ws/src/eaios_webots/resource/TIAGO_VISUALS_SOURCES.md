...
See `TIAGO_VISUALS_LICENSE-APACHE-2.0.txt`. Binary files under `meshes/` and
`textures/` are prepared locally and are not tracked by Git.
...
Before building or starting Soma, every path in
...
loads these local files and transfers them with the URDF.
